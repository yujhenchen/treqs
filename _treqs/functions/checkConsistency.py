...
class Consistency(object):
    def __init__(self):
        self.useRequirement = UseRequirement.UseRequirement()
    ...
    def setDataframe(self):
        self.df_cur = self.useRequirement.getContent(self.req_cur)
    ...
    def creatLogFile(self):
        str_date = commonFuns.getDateTimeStr()
                ...
                for filename in filenames:
                    ...
                    if match:
                        ...
                        with open(entry, "r") as file:
                            self.useRequirement.readContent(self.req_cur, commonFuns.readFile(entry))
                            df = self.useRequirement.getContent(self.req_cur)
                            # add file name column
                            df[param.file_name] = entry
                            appended_data.append(df)
            if len(appended_data) > 0:
                df_all = pd.concat(appended_data)
            self.useRequirement.setContent(self.req_cur, df_all)
            return True
        except Exception:
            ...
            for root, directories, filenames in os.walk(testcode_dir):
                for filename in filenames:
                    if filename.endswith((".py", ".json", ".cpp", ".java")):
                        ...
                        self.useRequirement.readContent(self.req_cur, commonFuns.readTestcodeElements(entry))
                        df = self.useRequirement.getContent(self.req_cur)
                        # add file name column
                        df[param.file_name] = entry
                        appended_data.append(df)
    ...
    def queryEmptyValue(self):
        try:
            ...
            if len(df.index) == 0:
                ...
                commonFuns.writeFileAppend(self.log_path, "=== requirements value missing ===", df.to_string())
                return False
        except Exception:
    ...
    def queryDuplicate(self):
        try:
            ...
            if len(df.index) == 0:
                ...
                # write dataframe into log files
                commonFuns.writeFileAppend(self.log_path, "=== requirements id duplicate ===", df.to_string())
                return False
        except Exception:
    ...
    def querySelfMissing(self):
        try:
            ...
            if len(df.index) == 0:
                ...
                # write dataframe into log files
                commonFuns.writeFileAppend(self.log_path, "=== requirements link_element missing ===", df.to_string())
                return False
        except Exception:
    ...
    def queryOtherMissing(self, requirement_link, column_link):
        if requirement_link == "" or column_link == "":
            return True
        try:
            df_link = self.useRequirement.getContent(requirement_link)
            # find link_element not in uid
            ...
            if len(df.index) == 0:
                ...
                # write dataframe into log files
                commonFuns.writeFileAppend(self.log_path, "=== requirements "+column_link+" missing ===", df.to_string())                
                return False
        except Exception:
...
def check():
    ...
    consist = Consistency()
    req_dir = input("Requirements files directory:")
